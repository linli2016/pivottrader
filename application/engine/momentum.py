...
from application.engine.setups import (
    detect_vcp,
...
class MomentumEngine:
    ...
    def calculate_and_store_momentum_metrics(self) -> None:
        ...
        4. For the latest date candidates, calculate VCP, EP, and Parabolic metrics in python, and store.
        """
        query_update_historical = """
            ...
            price_lags_base AS (
                SELECT 
                    ...
                    AVG(close * volume) OVER (PARTITION BY symbol ORDER BY date ROWS BETWEEN 49 PRECEDING AND CURRENT ROW) as dollar_vol_50d_ma,
                    GREATEST(
                        high - low,
            ...
            price_lags_derived AS (
                SELECT
                    ...
                    LAG(sma_200, 80) OVER (PARTITION BY symbol ORDER BY date) as sma_200_80d_ago,
                    LAG(sma_200, 100) OVER (PARTITION BY symbol ORDER BY date) as sma_200_100d_ago,
            ...
            SET
                ...
                dollar_vol_50d_ma = src.dollar_vol_50d_ma,
                adr_20d = src.adr_20d,
                atr_20d = src.atr_20d,
                ret_1m = src.ret_1m,
                ret_3m = src.ret_3m,
    ...
    def get_momentum_candidates(self, min_price: float = 5.00, min_vol_sma: int = 100000, min_dollar_vol: float = 10000000.0, min_rank: int = 70) -> List[Dict[str, Any]]:
        """
        Retrieves candidates passing price, volume, dollar volume, and momentum rank thresholds from daily_bars
        ...
        candidates = []
        with self.get_connection() as conn:
            res = conn.execute(query, [min_price, min_vol_sma, min_dollar_vol, min_rank]).fetchall()
            for row in res:
                candidates.append({
                    "symbol": row[0],
                    ...
                    "close": row[2],
                    "vol_50d_ma": row[3],
                    "dollar_vol_50d_ma": row[4],
                    "rs_score": row[5],
                    "rs_rank": row[6],
                    "atr_20d": row[7],
                    "pp_runup_pct": row[8],
                    "pp_drawdown_pct": row[9],
                    "sma_50": row[10],
                    "sma_150": row[11],
                    "sma_200": row[12],
                    "vcp_is_setup": bool(row[13]) if row[13] is not None else False,
                    "vcp_troughs": row[14],
                    "vcp_depths": row[15],
