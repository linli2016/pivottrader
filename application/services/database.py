...
class DatabaseService:
    ...
    def get_market_monitor(self, limit: int = 252, force_refresh: bool = False, as_of_date: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            with self.get_read_only_conn() as conn:
                ...
                cross_asset = self.get_cross_asset_data(conn, as_of_date=as_of_date, force_refresh=force_refresh)
                pulse_data = get_market_pulse(conn, as_of_date=as_of_date, force_refresh=force_refresh)
    ...
    def get_market_pulse_data(self, as_of_date: Optional[str] = None, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    def get_cross_asset_data(
        ...
        try:
            from application.services.cross_asset_service import cross_asset_service
            return cross_asset_service.get_cross_asset_data(conn=conn, as_of_date=as_of_date, force_refresh=force_refresh)
        except Exception as e:
            logger.warning(f"Error in cross_asset_service: {e}")
            from application.services.cross_asset_service import DEFAULT_BASE_ASSETS
            import copy
            return copy.deepcopy(DEFAULT_BASE_ASSETS)
